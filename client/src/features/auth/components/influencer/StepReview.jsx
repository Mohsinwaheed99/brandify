...
export default function InfluencerStepReview({ onBack, onSubmit }) {
  const { influencerData } = useAuthFlow();
  const [agreed, setAgreed] = useState(false);

  const rows = [
    ['Name', `${influencerData.firstName} ${influencerData.lastName}`],
    ['Email', influencerData.email],
    ['Phone', influencerData.phone],
    ['Niche', influencerData.niche],
    ['Instagram', influencerData.instagramConnected ? 'Connected' : 'Not connected'],
  ];
  ...
  return (
    <div>
      ...
      <label className="my-[22px] mb-1 flex items-start gap-2.5 text-[13px] text-ink-soft">
        ...
        I agree to CollabHub's Terms of Service and Creator Guidelines.
      </label>
